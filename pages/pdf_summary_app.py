...
def summarize_PDF_file(pdf_file,lang,trans_checked):
    if (pdf_file is not None):
        ...
        for page in reader.pages:
            ...
            text_summary = my_text_sum_12.summarize_text(page_text, lang)
            text_summaries.append(text_summary)
            
        token_num, final_summary = my_text_sum_12.summarize_text_final(text_summaries,lang)
        
        if final_summary != "":
            shorten_final_summary = textwrap.shorten(final_summary,250,placeholder="[이하생략...]")
            st.write("- 최종 요약(축약) : ",shorten_final_summary)
            st.write("- 최종 요약 : ",final_summary)
            ...
            if trans_checked:
                ...
                st.write("- 한국어 요약 : ",shorten_trans_result)
        else :
